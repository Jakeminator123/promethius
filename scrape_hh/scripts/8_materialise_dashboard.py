...
SELECT 
    ...
    ROUND(SUM(CASE WHEN a.action!='f' AND a.street='preflop' THEN 1 ELSE 0 END)*100.0 / 
          NULLIF(SUM(CASE WHEN a.street='preflop' THEN 1 ELSE 0 END), 0), 1)  AS avg_vpip,
    ROUND(SUM(CASE WHEN a.action='r' AND a.street='preflop' THEN 1 ELSE 0 END)*100.0 / 
          NULLIF(SUM(CASE WHEN a.street='preflop' THEN 1 ELSE 0 END), 0), 1)  AS avg_pfr,
    AVG(a.j_score)                                  AS avg_j_score,
    COUNT(a.action_order)                           AS total_actions,
    -- Handle both old (0-1) and new (1-100) preflop scores
    ROUND(AVG(CASE 
        WHEN a.preflop_score IS NOT NULL AND a.preflop_score <= 1 THEN a.preflop_score * 100
        ELSE a.preflop_score 
    END), 1)                                        AS avg_preflop_score,
...
SELECT
    ...
    /* Solver precision % – Y vs (Y+N), ignorerar NULL */
    ROUND(
        SUM(CASE WHEN UPPER(a.solver_best)='Y' THEN 1 ELSE 0 END) * 100.0 /
        NULLIF(SUM(CASE WHEN UPPER(a.solver_best) IN ('Y','N') THEN 1 ELSE 0 END),0)
    ,1) AS solver_precision_score
...
WITH base AS (
    SELECT
        ...
        COUNT(CASE WHEN UPPER(a.solver_best) IN ('Y','N') THEN 1 END)      AS solver_cnt,
        COUNT(CASE WHEN UPPER(a.solver_best)='Y' THEN 1 END)               AS solver_yes_cnt,
...
CREATE INDEX IF NOT EXISTS idx_actions_size_frac     ON actions(size_frac);
"""
...
def rebuild_tables(con: sqlite3.Connection) -> None:
    ...
    try:
        ...
        cur.executescript("""
            ...
            CREATE INDEX idx_ps_hands_played   ON player_summary(hands_played DESC);
            CREATE INDEX idx_ps_total_actions  ON player_summary(total_actions DESC);
        """)

...
# ─── 0. hitta heavy_analysis.db automatiskt ─────────────────────────
HERE = Path(__file__).resolve().parent        # …/scrape_hh/scripts
DB_NAME = "heavy_analysis.db"

def find_heavy_db(start: Path) -> Path | None:
    """Gå uppåt och returnera första <dir>/local_data/database/heavy_analysis.db."""
    for anc in [start] + list(start.parents):
        cand = anc / "local_data" / "database" / DB_NAME
        if cand.exists():
            return cand
    return None

DEFAULT_DB = find_heavy_db(HERE)
if DEFAULT_DB is None:
    sys.exit(f"❌ hittar ingen {DB_NAME} i någon <root>/local_data/database/ – "
             f"ange -db manuellt eller kontrollera mappstrukturen.")

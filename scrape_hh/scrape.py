...
from utils.paths import PROJECT_ROOT, POKER_DB, LOG_DIR, IS_RENDER, HEAVY_DB
...
def bulk_from_objects(con: sqlite3.Connection, hands: Iterable[dict[str, Any]]):
    ...
    for h in hands:
        ...
        # Hämta datum från hand-objektet
        # Försök först med 'date' fält, annars ta datum från config
        hand_date = h.get("date", CFG.get("STARTING_DATE", datetime.now().strftime("%Y-%m-%d")))
        
        meta_rows.append((
            h["stub"],
            hand_date,  # Använd faktiskt datum istället för h["stub"][:10]
        ...
        if h.get("partial_scores"):
            # Om chip_value redan normaliserats i _normalize_hand_chip_value,
            # är partial_scores redan normaliserade och vi kan spara dem direkt
...
def find_processing_scripts() -> List[Path]:
    ...
    # Lägg till optimize_indexes.py först om den finns
    optimize_script = scripts_dir / "optimize_indexes.py"
    if optimize_script.exists():
        scripts.append(optimize_script)
    
    # Sen alla numrerade scripts
    ...
    # Om vi har optimize_indexes, behåll den först, sen sortera resten
    if scripts and scripts[0] == optimize_script:
        return [scripts[0]] + sorted(scripts[1:])
    else:
...
def run_processing_scripts(skip_scripts: List[str] | None = None) -> bool:
    ...
    for script in scripts:
        ...
        try:
            is_script7 = "7_input_scores" in script_name
            timeout = 1800 if is_script7 else 300  # 30 min för script 7

            if is_script7:
                # Direkt-streama output så användaren ser progress
                # VIKTIGT: Lägg till --normalize för att få scores i rätt skala
            ...
            if result.returncode == 0:
                ...
                if not is_script7 and result.stderr:
...
# ────────────────────────────────────────────────────────────────
def checkpoint_databases() -> None:
    """Tvingar WAL-checkpoint(TRUNCATE) på både poker.db och heavy_analysis.db."""
    for db_path in (POKER_DB, HEAVY_DB):
        try:
            if db_path.exists():
                ...
                con.close()
                print(f"   🧹 Checkpoint: {db_path.name}")
        except Exception as e:
            print(f"   ⚠️  Checkpoint misslyckades för {db_path.name}: {e}")

# ────────────────────────────────────────────────────────────────
# 0b. helper: normalisera chip_value
# ────────────────────────────────────────────────────────────────

def _normalize_hand_chip_value(hand: dict[str, Any]) -> None:
    """Skalar alla chip-relaterade belopp så att chip_value_in_displayed_currency blir 1.
    Modifierar hand-objektet in-place."""
    cv = hand.get("chip_value_in_displayed_currency")
    if not cv or cv == 1:
        return  # ingen normalisering behövs

    try:
        factor = 1 / float(cv)
    except (ValueError, ZeroDivisionError):
        return  # går inte tolka – hoppa över
    ...
    # Skala blinds och ante
    for k in ("big_blind_amount", "small_blind_amount", "ante_amount"):
        if k in hand and hand[k] is not None:
            hand[k] = _scale(hand[k])

    # Skala effektiva stackar
    if hand.get("effective_stack") is not None:
        hand["effective_stack"] = round(float(hand["effective_stack"]) * factor, 3)

    # Position-specifika belopp
    for pos in hand.get("positions", {}).values():
        if "stack" in pos:
            pos["stack"] = _scale(pos["stack"])
        if "money_won" in pos and pos["money_won"] is not None:
            pos["money_won"] = round(float(pos["money_won"]) * factor, 3)

    # Skala belopp i breadcrumb (format "…r1250" etc)
    if hand.get("breadcrumb"):
        hand["breadcrumb"] = re.sub(r"r(\d+)",
                                     lambda m: f"r{_scale(m.group(1))}",
                                     hand["breadcrumb"])

    # Skala belopp i situation_string och preflop
    for k in ("situation_string", "preflop"):
        if hand.get(k):
            hand[k] = re.sub(r"r(\d+)",
                             lambda m: f"r{_scale(m.group(1))}",
                             hand[k])

    # Skala nycklar i partial_scores
    if hand.get("partial_scores"):
        new_ps = {}
        for node, val in hand["partial_scores"].items():
            new_key = re.sub(r"r(\d+)",
                             lambda m: f"r{_scale(m.group(1))}",
                             node)
            new_ps[new_key] = val
        hand["partial_scores"] = new_ps

    # Uppdatera chip_value till 1
    hand["chip_value_in_displayed_currency"] = 1
...
def main() -> None:
    ...
    for seq, hand in api.iter_hands(date):
        ...
        # ── Normalisera chip_value före vidare hantering ──
        _normalize_hand_chip_value(hand)
        ...
        # Lägg till datum i hand-objektet för bulk_from_objects
        hand["date"] = date
        objs.append(hand)
        ...
        if len(rows) >= batch_size:
            ...
            # WAL-checkpoint efter varje batch
    ...
    if rows:
        ...
        if not args.no_scripts:
            scripts_ok = run_processing_scripts(args.skip_scripts)
            if not scripts_ok:
                print(f"❌ Scripts misslyckades i sista batch - avbryter")
                return

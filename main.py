...
def force_cleanup_on_start() -> None:
    ...
    # Om FORCE_CLEAN=true -> ta bort alla databaser oavsett markerfil
    if os.environ.get("FORCE_CLEAN", "false").lower() == "true":
        try:
            deleted = _delete_all_database_files()
            print(f"🧨 FORCE_CLEAN aktiv: raderade {deleted} databas-filer")
        except Exception as e_fc:
            print(f"⚠️  FORCE_CLEAN fel: {e_fc}")
...
def handle_first_deploy() -> bool:
    ...
    if marker_file.exists() and os.environ.get("FORCE_CLEAN", "false").lower() != "true":
        print("♻️  Kontinuerlig drift - behåller befintlig data (FORCE_CLEAN=false)")
        try:
...
def _start_webserver(*, reload: bool = False) -> None:
    """Startar Uvicorn som huvudprocess."""
    import uvicorn

    port = int(os.environ.get("PORT", 8000))

    env = "Render" if IS_RENDER else "Local"
    print(f"🌐 Webserver ({env}) på port {port}")

    # Öppna browser automatiskt i lokal miljö
    if not IS_RENDER:
        try:
            import webbrowser, threading, time

            def _open_browser() -> None:
                # Vänta kort så att servern hinner binda porten
                time.sleep(1)
...
# ────────────────────────────────────────────────────────────────
# Lokal utvecklings-miljö (scraping + webserver)
# ────────────────────────────────────────────────────────────────
...
def start_local_environment(
    ...
    """Startar scraping-loop i bakgrund + webserver med autoreload lokalt."""

    def run_scraping_background() -> None:
        print("🔄 (Local) Startar scraping-loop i bakgrund …")
        print("   🧵 Scraping-tråd startad!")
        time.sleep(2)  # Ge lite tid för output
        try:
            run_scraping_loop(
                start_date,
                url,
            ...
            print(f"❌ Scraping-tråden kraschade: {e}")

    scraping_thread = threading.Thread(target=run_scraping_background, daemon=True)
    scraping_thread.start()
    print("✅ Scraping-tråd skapad och startad")
    time.sleep(1)  # Ge tråden tid att starta

    # Säkerställ databaser finns för läsning
    _ensure_databases_exist()
    print("📦 Databaser kontrollerade")

    # Starta Uvicorn med reload för bekväm utveckling
    print("🌐 Startar webserver med autoreload...")
    _start_webserver(reload=True)
...
def main() -> None:
    ...
    if IS_RENDER:
        ...
        # Lokal utveckling: scraping + webserver med autoreload
